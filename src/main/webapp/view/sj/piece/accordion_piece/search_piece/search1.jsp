   ...
   pageEncoding="UTF-8"%>
            ...
            <div class="col-md-6">
               ...
               <div class="input-group bootstrap-timepicker">
                  <input type="text" class="form-control timepicker-default" id="keyword">
                  <span class="input-group-btn">
                     <button class="btn btn-theme04" onclick="search()">검색</button>
                  </span>
               </div>
...
<!-- 저장-------------------------------------------------------------------------------- -->
   <form action="${pageContext.request.contextPath}/sj/counter/" method="post" class="sjurl">
               ...
               <input type="hidden" name="the_code" value="1"  readonly><!--아점저코드  -->
               <input type="hidden" name="id" value="${sessionScope.loginUser.mem_id}"  readonly><!-- 아이디 -->  
               <button class="btn btn-theme col-md-1 col-md-offset-11" style="margin-top: 15px;">식단추가</button>
   </form>
 <!-- /저장-------------------------------------------------------------------------------- -->
               <!--/ 음식추가 -->            
            cally
            ...
            for (var i = 0; i < item.length; i++) {
               $(".result").append(
                     ...
                     +$(DESC_KOR[i]).text()+'('+$(item.find("ANIMAL_PLANT")[i]).text()+')'
                      ...
                      +'</b><b>Cal</b></div><div class="col-md-12"><h4 ><div class="col-md-6"><input type="text" class="form-control content" value="1" id="content"></div>'
...
<script>
/* 동적생성 이벤트 */
$(document).on("keyup","table.Gram tr",function(){  
   ...
   $(" td div").keyup(function(){    
    ...
    $(document).on("change","table.Gram tr",function(){
    	 var thiss = $(this); 
         ...
         var ANIMAL_PLANT =  $(this).find("td input#ANIMAL_PLANT").val(); //
           ...
           if ($(".checked").is(':checked')) {
              ...
              if($("#div_List  input.NUTR_CONT1").val() == 'N/A' ){$("#div_List  input.NUTR_CONT1").val("0");
              }if($("#div_List  input.NUTR_CONT2").val() == 'N/A' ){$("#div_List  input.NUTR_CONT2").val("0");
              }if($("#div_List  input.NUTR_CONT3").val() == 'N/A' ){$("#div_List  input.NUTR_CONT3").val("0");
              }if($("#div_List  input.NUTR_CONT4").val() == 'N/A' ){$("#div_List  input.NUTR_CONT4").val("0");
              }if($("#div_List  input.NUTR_CONT5").val() == 'N/A' ){$("#div_List  input.NUTR_CONT5").val("0");
              }if($("#div_List  input.NUTR_CONT6").val() == 'N/A' ){$("#div_List  input.NUTR_CONT6").val("0");
              }if($("#div_List  input.NUTR_CONT7").val() == 'N/A' ){$("#div_List  input.NUTR_CONT7").val("0");
              }if($("#div_List  input.NUTR_CONT8").val() == 'N/A' ){$("#div_List  input.NUTR_CONT8").val("0");
              }if($("#div_List  input.NUTR_CONT9").val() == 'N/A' ){$("#div_List  input.NUTR_CONT9").val("0");}        
