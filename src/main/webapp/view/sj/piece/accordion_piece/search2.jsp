   ...
   pageEncoding="UTF-8"%>
            ...
            <div class="col-md-6">
               ...
               <div class="input-group bootstrap-timepicker">
                  <input type="text" class="form-control timepicker-default" id="keyword2">
                  <span class="input-group-btn">
                     <button class="btn btn-theme04" onclick="search2()">검색</button>
                  </span>
               </div>
               ...
               <div class="col-md-12" style="overflow-y: scroll;height: 350px;top: 15px;">
                  <div class="white-panel">
                     <div class="custom-check goleft mt">
                        ...
                        <!--검색결과------------------------------------------------------------------- -->
                        <form name="myform">
                        <table id="todo " class="table table-hover custom-check Gram2" >
...
<!-- 저장-------------------------------------------------------------------------------- -->
   <form action="${pageContext.request.contextPath}/sj/counter/" method="post" class="sjurl">
               ...
               <input type="hidden" name="the_code" value="2"  readonly><!--아점저코드  -->
...
$.get("${pageContext.request.contextPath}/sj/foodsdb", {
                     desc_kor : desc_kors,
                        ...
                        console.log(obj);
                         ...
                         if( obj == '' ){
                        $(".result2").append("<p>db 검색 결과 없음</p>");
...
/*--api---------------------------------------------------------------------------------------------  */                        
      $.get("${pageContext.request.contextPath}/sj/xml", {
         ...
         if (item.length == 0) {            
            ...
            $(".result2").append("<p>검색어가 없습니다!</p>");
            $(".result2").append('<a href="${pageContext.request.contextPath}/sj/Ingredient_up">새등록하기!</a>');
            /* alert('식품데이터 없음'); */
...
/*--api---------------------------------------------------------------------------------------------  */
                        ...
                        $(".result2").append("<p>db 검색 결과 있음</p>");
                        $(".result2").append('<a href="${pageContext.request.contextPath}/sj/Ingredient_up">새등록하기!</a>');
                        $(".result2").append(obj);
                     }
                  });
...
<script>
/* 동적생성 이벤트 */
$(document).on("keyup","table.Gram2 tr",function(){  
   ...
   $(" td div").keyup(function(){    
    ...
    $(document).on("change","table.Gram2 tr",function(){
    	 var thiss = $(this); 
         ...
         var ANIMAL_PLANT =  $(this).find("td input#ANIMAL_PLANT").val(); //
           ...
           if ($(".checked").is(':checked')) {
              ...
              if($("#div_List2  input.NUTR_CONT1").val() == 'N/A' ){$("#div_List2  input.NUTR_CONT1").val("0");
              }if($("#div_List2  input.NUTR_CONT2").val() == 'N/A' ){$("#div_List2  input.NUTR_CONT2").val("0");
              }if($("#div_List2  input.NUTR_CONT3").val() == 'N/A' ){$("#div_List2  input.NUTR_CONT3").val("0");
              }if($("#div_List2  input.NUTR_CONT4").val() == 'N/A' ){$("#div_List2  input.NUTR_CONT4").val("0");
              }if($("#div_List2  input.NUTR_CONT5").val() == 'N/A' ){$("#div_List2  input.NUTR_CONT5").val("0");
              }if($("#div_List2  input.NUTR_CONT6").val() == 'N/A' ){$("#div_List2  input.NUTR_CONT6").val("0");
              }if($("#div_List2  input.NUTR_CONT7").val() == 'N/A' ){$("#div_List2  input.NUTR_CONT7").val("0");
              }if($("#div_List2  input.NUTR_CONT8").val() == 'N/A' ){$("#div_List2  input.NUTR_CONT8").val("0");
              }if($("#div_List2  input.NUTR_CONT9").val() == 'N/A' ){$("#div_List2  input.NUTR_CONT9").val("0");}        
    ...
    $(document).on("click","tbody#div_List2 tr",function(){
    	var click = $(this);	
